...
class NameUtil{
    ...
    static public $NAME_CAMEL_SINGULAR  = "CAMEL_SINGULAR";     //驼峰法单数
    static public $NAME_CAMEL_PLURAL    = "CAMEL_PLURAL";       //驼峰法复数
    ...
    public function __construct($name){
        ...
        $this->names[self::$NAME_CAMEL_SINGULAR] = "";
        $this->names[self::$NAME_CAMEL_PLURAL] = "";
        foreach ($els as $index => $el){
            if($index === 0){
                ...
                $this->names[self::$NAME_CAMEL_SINGULAR] = $el;
                $this->names[self::$NAME_CAMEL_PLURAL] = $el . "s";
            } else {
                $this->names[self::$NAME_PASCAL_SINGULAR] .= ucfirst($el);
                ...
                $this->names[self::$NAME_CAMEL_SINGULAR] .= ucfirst($el);
                $this->names[self::$NAME_CAMEL_PLURAL] .= ucfirst($el);
            }
        }
    }
     ...
     * @return string
     */
    public function getCamel($isPlural = false)
    {
        return $isPlural ? $this->names[self::$NAME_CAMEL_PLURAL] : $this->names[self::$NAME_CAMEL_SINGULAR];
    }
}
